...
mismatches between declared and executed strategies. Each of these failure modes corresponds to a distinct layer of the
generation process and requires a targeted evaluation method. Variable
strategy selection maps to plan-level measurement (\autoref{sec:meth-m1}),
variable response content to semantic comparison (\autoref{sec:meth-m2}),
and plan-response mismatch to alignment verification
(\autoref{sec:meth-m3}).
...
Despite these requirements, stability remains largely unmeasured. Prior
...
therapeutic consistency. Factual consistency studies measure whether models
produce the same answers to knowledge questions across runs. Output
diversity research quantifies lexical and semantic variation in creative
generation. Benchmark reproducibility work examines whether evaluation
scores are stable across random seeds.
...
These lines of research share the concern for cross-run variation but do
not address whether a model makes the same \emph{clinical decisions} when
presented with identical patient contexts.
