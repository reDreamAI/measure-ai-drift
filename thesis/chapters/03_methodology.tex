...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_3_1_pipeline.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_3_3_eval_stack.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_3_2_measurement.pdf}
...
Preliminary testing on this task showed an F1 ceiling of approximately
0.55, insufficient for reliable scoring. Cross-encoders also produce no
reasoning trace, making it impossible to audit individual judgements.
...
caveat explicitly. In addition, 12 trials with the lowest alignment
scores are manually reviewed to check whether the judge penalised
valid therapeutic responses. A score of zero indicates a failed judge
output rather than a genuine misalignment. In all observed cases, a
single rerun of the judge call was sufficient to produce a valid score,
so zero scores are treated as transient failures and rerun once before
inclusion.

As reported in \autoref{sec:res-alignment}, alignment scores are near
ceiling across most models and temperature-independent. This means
alignment serves primarily as a validation check (models consistently
implement what they declare) rather than a comparative metric. This
outcome validates Method~1: because plans are faithfully executed,
Jaccard over plan sets measures genuine decision stability rather than
label noise.
...
this complexity, and including three Qwen scales (27B, 122B, 397B) enables
...
\begin{itemize}
  \item $T = 0.0$ (greedy decoding): deterministic upper bound on stability.
    No reproducibility seed is set, as not all vendors support one.
    Even where a seed is available, GPU parallelisation and dynamic
    batching introduce floating-point non-determinism that makes
    output-level reproducibility best effort at most
    \parencite{HITR7PL4}. The twenty-trial repetition design captures
    this variance directly, following \textcite{WA4QD3UE}, who use the
    same 20-trial protocol at $T = 0.0$ to quantify \gls{llm} stability.
